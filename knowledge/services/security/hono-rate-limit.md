...
description: "Custom rate-limit middleware in the Hono Worker, sliding-window per-IP via Cloudflare Workers KV. Free — runs on existing Worker + KV free tiers, no card. Fine-grained per-route throttling that complements the coarser zone-level WAF rate-limit."
...
([decision](../../decisions/security/anti-bot-defense-in-depth.md))
