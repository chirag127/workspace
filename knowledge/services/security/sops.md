...
description: "SOPS is the family's git-native secrets encryption tool. Encrypts values inside structured files (YAML/JSON/ENV/INI) while keeping keys + structure visible for readable diffs. Originally Mozilla (2015), donated to CNCF Sandbox 2023, now maintained at getsops/sops (the mozilla/sops repo redirects). Current version v3.13.1 (May 2026). Used with [[services/security/age]] as the encryption backend, no cloud key-server needed."
...
We use [`age`](https://github.com/FiloSottile/age) (`services/security/age`) as the master-key backend. Reasons in priority order:
...
CI uses `SOPS_AGE_KEY` env var with the private key contents — see [[decisions/security/sops-plus-doppler-hybrid]].
...
- [[services/security/age]] — the backend that does the actual key wrapping
- [[decisions/security/sops-plus-doppler-hybrid]] — why SOPS+age is source-of-truth and Doppler is runtime-sync only
- [[rules/submodule-env-files-three-file-pattern]] — every submodule that has secrets gets a `.env`, `.env.enc`, `.env.example`
- [[decisions/policy/private-repos-excluded-from-mirror-cron]] — protects the encrypted blobs from public mirror cron just in case
