...
Three roles across two vendors: **Spaceship** is the registrar of record (the user's existing registrar), **Cloudflare** hosts DNS via NS delegation, and **Cloudflare Email Routing** forwards every `*@oriz.*` address to a single Gmail inbox. The full architecture is locked at [`decisions/infrastructure/spaceship-registrar-cloudflare-dns.md`](../../decisions/infrastructure/spaceship-registrar-cloudflare-dns.md).
