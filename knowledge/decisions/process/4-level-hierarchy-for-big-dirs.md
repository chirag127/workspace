...
description: services/, decisions/, glossary/ use 4-level paths
...
- Cross-link references that used `services/firebase-spark.md` now resolve to `services/business/auth/firebase-spark.md`. All in-bundle links updated in the same edit; the [self-update rule](../../rules/agent/self-update-rule.md) carries the obligation.
- New service / decision / glossary files MUST land in the appropriate subdir, not at the root.
- Per-app `knowledge/` bundles (e.g. `repos/oriz/own/prod/apps/personal/oriz-cs-me-app/knowledge/`, inside each submodule) MAY adopt 4-level once they hit the same threshold. Until then, they stay flat.
...
- [rules/self-update-rule](../../rules/agent/self-update-rule.md)
