...
description: "Sops+age remains the source of truth for the family's 65-key .env (preserves 134 lines of section comments + ordering that Doppler's flat KV model would destroy). Doppler is added as a parallel runtime-sync layer for CI fan-out only: a local script does `doppler secrets upload .env --silent` after each local .env edit, and Doppler's GitHub Actions integration pushes to oriz-org with sync_target=org (1 sync, free tier). Bitwarden CLI keeps the age key. Doppler activity log (3-day retention) is a nice-to-have, NOT the source of truth — `.env.enc` in git is."
...
- env single-source rule: [[decisions/security/env-single-source-auto-push]]
- Org-level secrets rule: [[rules/org-level-secrets-only-no-per-repo]]
- No-card-on-file constraint: [[rules/no-card-on-file]]
- The org we depend on: [[decisions/branding/oriz-org-rename-from-co]]
