...
description: "Every family site ships all three SEO pillars: @astrojs/sitemap (discovery), IndexNow (instant indexing), and JSON-LD structured data (semantic). Submitted to Google Search Console + Bing Webmaster Tools. All free, all no-card."
...
- **`oriz-omnipost` adds an `IndexNowAdapter`**: alongside the
  ...
  RSS `<guid>` per
  [`decisions/architecture/cross-post-engine.md`](./cross-post-engine.md).
...
- [decisions/architecture/cross-post-engine.md](./cross-post-engine.md) — oriz-omnipost fires IndexNow
