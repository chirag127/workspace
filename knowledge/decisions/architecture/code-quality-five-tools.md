---
type: decision
title: "Code quality — 5-tool stack (Sonarcloud + CodeRabbit + Codecov + Code Climate + DeepSource)"
description: "Locked 2026-06-20: every public repo runs five complementary code-quality tools. Sonarcloud (SAST + smells), CodeRabbit (LLM PR review), Codecov (coverage delta), Code Climate (A — F maintainability), DeepSource (autofix). All five free for the family's public / OSS repos. Builds on the earlier 4-tool stack — adds Codecov + Code Climate + DeepSource alongside the existing Dependabot + biome + CodeRabbit + Sonarcloud."
