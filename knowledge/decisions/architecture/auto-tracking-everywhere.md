...
- **Everything else in the stack is already auto.** Errors
  ...
  closes that gap. See
  [`time-tracking-wakatime-only`](./time-tracking-wakatime-only.md).
...
- **Hookdeck quota** (50K events/mo free, per
  [`distribution-and-queues-locked`](./distribution-and-queues-locked.md))
...
- **CF Worker 100K req/day** — every ingest endpoint sits
  behind the [CF Worker quota mitigation playbook](./cf-worker-quota-mitigation.md);
