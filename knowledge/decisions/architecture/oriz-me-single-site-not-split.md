---
type: decision
title: "oriz-me-site stays a single site with sections — not split into now/uses/gear/cv subdomains"
description: "me.oriz.in stays one Astro site with internal URL sections (/now, /uses, /gear, /reading, /coding, /lifestream, /cv, /contact). Not split into now.oriz.in, uses.oriz.in, gear.oriz.in, etc."
...
3. **Locked architecture already assumes one store.** The lifestream
   JSONL canonical file (per [`lifestream-jsonl-canonical`](./lifestream-jsonl-canonical.md))
...
- DO NOT split the lifestream JSONL across multiple sites — one site,
  one canonical store, per [`lifestream-jsonl-canonical`](./lifestream-jsonl-canonical.md).
...
- [Lifestream JSONL canonical](./lifestream-jsonl-canonical.md) — single canonical store across the site
