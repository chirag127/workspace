...
`me.oriz.in` AND `cs.oriz.in` — both DNS-aliased to the same site (`projects/apps/personal/oriz-cs-me-app/`). Two URLs, one app, identical content. `me.oriz.in` is the canonical; `cs.oriz.in` is the personal-initials variant.
...
- Per-app knowledge bundle for this app (richest in the family) → `projects/apps/personal/oriz-cs-me-app/knowledge/`
