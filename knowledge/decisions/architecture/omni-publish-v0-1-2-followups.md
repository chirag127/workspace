...
description: "Five follow-ups deferred from @chirag127/omni-publish v0.1.1 → v0.1.2: per-repo per-day rate-limit cache (high), retry on transient 5xx (medium), compile TS → dist/ for non-bundler consumers (medium), Hashnode tag _id resolution (low), Threads single-user-token assumption validation (low)."
tags: [decision, package, omni-publish, followups, v0-1-2, technical-debt]
...
Threshold for the cache: **max 1 auto-post per channel per day per repo** per [`revenue-channels-2026`](./revenue-channels-2026.md).
