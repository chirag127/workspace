...
| [site-rename-matrix.md](./site-rename-matrix.md) | Site rename matrix — drop `oriz-` prefix, add `-site` suffix on all kept submodule sites. `gh repo rename` preserves stars/issues/forks/history. `oriz-finance` / `oriz-image-tools` / `oriz-pdf-tools` / `oriz-urls-to-md` absorbed into Tier 1 tool repos with gh-pages 301 redirects |
