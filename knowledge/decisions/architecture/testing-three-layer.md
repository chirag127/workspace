---
type: decision
title: "Testing — three-layer stack (Vitest unit + Playwright E2E + Storybook+Chromatic visual)"
description: "Every PR runs Vitest unit, Playwright E2E, and Chromatic visual-regression against Storybook in parallel. PR fails on any failure in any layer. All free, no card."
...
2. **End-to-end** — [Playwright](../../services/testing/playwright.md). Real
   ...
   [a11y axe-core suite](./a11y-three-tools.md) rides on.
3. **Visual regression** —
...
- **Reuses substrate already required by the
  [a11y trio](./a11y-three-tools.md).** Playwright is already
...
- **No "warn-only" gating.** Same logic as the
  [a11y trio](./a11y-three-tools.md): warn-only across 11+ sites is
...
- [a11y three-tools decision — rides on the same Playwright install](./a11y-three-tools.md)
