...
description: "Two AI providers, picked by surface. Puter.js for browser-side calls (user-pays, no API key client-side). Cloudflare Workers AI for server-side calls inside the Hono Worker (10K neurons/day, zero-egress, native binding). Different surfaces, different reasons."
...
- **Quota headroom** per [`rules/never-hit-quotas.md`](../../rules/never-hit-quotas.md):
