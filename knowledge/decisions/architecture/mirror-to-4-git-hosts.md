...
description: "Insurance against GitHub becoming unusable. Master umbrella runs a Friday-4am-IST cron that pushes every submodule's HEAD to GitLab.com + Codeberg.org + Bitbucket + GitFlic.ru. Each mirror is push-only (read-from-GH-write-to-mirror). If a mirror fails, others still succeed."
tags: [decision, mirror, insurance, git-host, backup, multi-platform]
