...
description: "Two API-mock tools, one per surface. MSW handles in-browser + in-Node test mocks (unit / Vitest, component stories, Playwright dev). Mockoon handles E2E + manual dev mocks of third-party APIs (Razorpay sandbox, Open-Meteo, Alpha Vantage when offline). Both free OSS. Different surfaces, different reasons."
...
- **Reuses substrate already required by other family decisions.**
  ...
  [testing three-layer decision](./testing-three-layer.md) already
...
- **Firestore via REST** — keep the live Firebase emulator if it's already on hand (`firebase emulators:start --only firestore`); MSW is a fallback for unit-test surfaces that don't want to spin the emulator. The
  [firebase-rest-firestore decision](./firebase-rest-firestore-not-admin.md)
...
- A drift between an updated upstream API shape and the mock is
  caught by the [test layers](./testing-three-layer.md) — Vitest
...
- [Testing three-layer decision](./testing-three-layer.md)
