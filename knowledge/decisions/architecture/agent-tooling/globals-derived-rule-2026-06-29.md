...
description: Workspace files are the canonical source of truth. Global agent configs (`~/.claude.json`, `~/.opencode/`, etc.) are derived caches written by `scripts/sync-globals.mjs`. The script fires grill-me when it detects drift or a new item. Reversed an earlier "workspace-only" framing (the same turn) after OpenCode broke on per-agent schema differences and you flagged that pure workspace-only would bloat the repo root.
