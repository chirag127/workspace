...
description: "Non-code posts get OG cards from Satori (@vercel/og) on a Hono Worker route at api.oriz.in/og. Code-heavy posts continue on ray.so. Static-cached via CF edge cache headers, no per-post PNGs in any site repo."
...
- **Stack cohesion.** Satori runs on the existing api.oriz.in Hono
  umbrella Worker per
  [`hono-worker-api-umbrella.md`](./hono-worker-api-umbrella.md) —
...
- **One Hono route owned**: `GET /og` on
  [`api.oriz.in`](./hono-worker-api-umbrella.md). Handler validates
...
- **Cross-poster awareness**: [`oriz-omnipost`](./cross-post-engine.md)
