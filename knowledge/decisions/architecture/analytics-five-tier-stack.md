...
description: "Locked 2026-06-20: every site runs five analytics layers in parallel — Cloudflare Web Analytics (raw load), Google Analytics 4 (marketing funnel), PostHog (product + session replay + flags), Microsoft Clarity (heatmaps + Microsoft-side session replay), UTM tracking (attribution convention). Each layer covered by an `ENABLE_<TOOL>` env-var kill-switch so no single quota pinch can break a site."
