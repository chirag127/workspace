...
description: "How to publish a userscript from oriz-org/userscripts to greasyfork.org. Greasy Fork has NO upload API — first version requires manual paste in the in-browser editor. After the initial paste, register a webhook so subsequent git push → automatic Greasy Fork bump. One-time per script. ~5 minutes."
