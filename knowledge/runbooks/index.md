...
| [`mirror-all-hosts-setup.md`](./mirror-all-hosts-setup.md) | **Current** one-time setup for the 6-host automatic mirror (GitLab, Codeberg, Bitbucket, GitFlic, Azure DevOps, AWS CodeCommit): token generation, pre-creating repos on all 6 hosts, storing 13 secrets at chirag127 org level, dry-run + first real run verification. See [`decisions/architecture/mirror-to-6-git-hosts.md`](../decisions/architecture/mirror-to-6-git-hosts.md) | User (token gen), agent (scripted repo creation + verify) |
