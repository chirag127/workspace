...
description: "After landing a feature inside a submodule, bump the master repo's pointer to it. The standard `feat in submodule, chore: bump in master` two-commit workflow."
...
2. **Verify you're on `main`.** Per
    [`one-branch-only.md`](../rules/one-branch-only.md), no other
    branch should exist.
...
- [`../rules/conventional-commits.md`](../rules/conventional-commits.md)
