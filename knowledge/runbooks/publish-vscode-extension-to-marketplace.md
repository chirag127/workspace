...
description: "How to ship an oriz-org/-vsc-ext repo to the VS Code Marketplace + Open VSX. One-time: create a Microsoft Publisher under your account, generate an Azure DevOps PAT with Marketplace scope, store it. Per-release: bump version, vsce package + publish + ovsx publish. ~5 min per release after setup."
...
- Family naming convention (where `-vsc-ext` comes from): [[decisions/branding/repo-naming-suffixes]]
