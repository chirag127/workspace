...
description: "Web3Forms server-side calls require their paid plan plus an IP allow-list. Cloudflare Workers' egress IPs rotate. Always submit Web3Forms from the browser."
tags: [rules, web3forms, contact-forms, cloudflare, workers]
...
[no-card-on-file](./no-card-on-file.md) rule AND would need a
subscription, which the [no-subscriptions](./no-subscriptions.md) rule
forbids anyway.
...
- [`no-card-on-file.md`](./no-card-on-file.md)
