...
description: "When a session needs structured questioning, invoke the grill-me skill rather than rolling inline multi-choice question prompt calls. Skill carries the ranked-options + recommendation discipline and stays consistent across sessions."
...
When a session calls for structured questioning (any non-trivial fork, any architectural decision, any "stress-test my plan" request), invoke the `grill-me` skill via the skill-invocation tool rather than rolling multi-choice question prompt calls inline.
...
- Inline multi-choice question prompt is still valid for one-off forks during normal work — the skill is for *sustained* questioning.
