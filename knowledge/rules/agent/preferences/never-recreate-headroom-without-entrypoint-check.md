...
description: The headroom-extras image's Docker ENTRYPOINT is already `headroom`. When `docker run` is used to recreate the container, the CMD must start with `proxy ...`, NOT `headroom proxy ...`. Passing `headroom` twice causes "unexpected extra arguments" and breaks the entire Claude Code → Hr → hai chain until manually restored.
