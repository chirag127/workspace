...
description: "Commits are sized to match the work unit: a single locked decision = one commit; a batch grill session (3+ related decisions) = one commit covering the batch; a multi-file refactor = one commit per logical refactor unit. Heuristic, not mechanical."
