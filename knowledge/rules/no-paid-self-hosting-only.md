...
description: "Reversal 2026-06-22: self-hosting is FINE as long as the provider is free / no-card-on-file. The hard constraint is 'no card on file', not 'no servers'. This rule is now effectively merged with no-card-on-file.md and kept as a thin pointer + provider allowlist."
tags: [rule, hosting, no-card, free-tier, self-hosting]
...
over-strict and is now merged with [`no-card-on-file.md`](./no-card-on-file.md).
...
- [`no-card-on-file.md`](./no-card-on-file.md) — the actual binding constraint
