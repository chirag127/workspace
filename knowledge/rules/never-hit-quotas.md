...
description: "Architect for headroom. Surprise quota walls are a design failure, not a billing event."
...
4. **Spread load across data-shape-specific stores** so no single
   quota becomes the bottleneck (see [layer-4](../architecture/layer-4-database-by-shape.md)).
