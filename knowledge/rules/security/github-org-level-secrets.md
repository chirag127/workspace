...
4. **Visibility = `all` for shared keys** (Sentry DSN, Doppler service
   ...
   <!-- TODO: broken link, was [`templates/.env.example`](../../templates/.env.example) --> by
   default).
...
6. **Doppler stays upstream.**
   ...
   and [`runbooks/security/set-github-org-level-secrets.md`](../../runbooks/security/set-github-org-level-secrets.md).
...
- **Setting / updating a secret.** Use the runbook
  [`runbooks/security/set-github-org-level-secrets.md`](../../runbooks/security/set-github-org-level-secrets.md).
  In summary:
...
- **Adding a new key.** Add it to
  <!-- TODO: broken link, was [`templates/.env.example`](../../templates/.env.example) --> first
...
- **Auditing.** Quarterly:
  ...
  <!-- TODO: broken link, was [`templates/.env.example`](../../templates/.env.example) -->. Drift in
...
- **Not** "every secret in Doppler is auto-synced to GH." Only the
  keys that appear in
  <!-- TODO: broken link, was [`templates/.env.example`](../../templates/.env.example) --> are
