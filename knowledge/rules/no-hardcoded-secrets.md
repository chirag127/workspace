...
description: "Secrets never live in source. All secrets come from chirag127/envpact-secrets, pulled at dev/build/CI time."
tags: [rules, secrets, security, envpact]
...
[`no-card-on-file.md`](./no-card-on-file.md) include the €54K Gemini
...
- Worker secrets in Cloudflare Secrets Store (per
  [`apps/api`](../architecture/api-umbrella-hono-worker.md)) — the
...
4. Full procedure at [`../runbooks/auth-setup.md`](../runbooks/auth-setup.md).
