...
description: "Every tracked metric in the chirag127/oriz family must be automatically captured. Manual entry, manual timer, manual journal not allowed for anything that's a system metric. Manual = decay; auto = honest. Applies to METRICS, not content."
...
- [`decisions/architecture/auto-tracking-everywhere.md`](../decisions/architecture/auto-tracking-everywhere.md) — the family-wide decision that locks this principle, with concrete auto-event source list for the lifestream
- [`decisions/architecture/time-tracking-wakatime-only.md`](../decisions/architecture/time-tracking-wakatime-only.md) — the walk-back of Toggl
- [`decisions/architecture/lifestream-jsonl-canonical.md`](../decisions/architecture/lifestream-jsonl-canonical.md) — the JSONL stream that consumes auto sources
