...
description: "Every CI workflow in every chirag127/oriz* repo runs on Linux/Ubuntu runners. macOS and Windows runners are forbidden unless explicitly justified for native build that can't be done another way. PWABuilder handles all native packaging from a Linux runner via the deployed PWA URL — no macOS/Windows runner needed."
tags: [rule, ci, linux, runners, cost]
