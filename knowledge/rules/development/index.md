...
- [Fork discipline — minimum diff, rebase-friendly, upstream-aligned](./fork-discipline.md) — Where forks live, layout, anchors, minimum-diff principle, divergence.md. Subsumes the old fork-customization-minimum-conflict rule (deleted 2026-06-28).
- [Fork discipline — minimum diff, rebase-friendly, upstream-aligned](./fork-discipline.md) — All forks live under chirag127/<upstream-name> on GitHub and repos/oriz/frk/<bucket>/<category>/<upstream-name>/ on disk. Reason: org-level secrets only propagate within the org; forks on personal accounts can't inherit them. Slug + disk folder = upstream name by default, unless the fork ships as a distinct product (CWS / store / npm) — then rename to a `<purpose>-bs-ext` / `-cli` / etc. compliant slug. All code changes minimum-diff, marked with per-fork `<slug>:` comments, documented in per-fork knowledge/divergence.md.
