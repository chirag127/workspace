...
|---|---|---|---|
| [Cloudflare Web Analytics](../../services/monitoring/monitoring/analytics/cloudflare-web-analytics.md) | `necessary` | Loaded always | Cookie-less by design; documented under necessary for legal clarity |
| [Sentry](../../services/monitoring/monitoring/sentry.md) | `necessary` (default) ? `analytics` if user-PII captured | Loaded with PII off by default | Default Sentry config does NOT capture PII; if a site flips on user-context, the entry moves to `analytics` for that site |
| [Google Analytics 4](../../services/monitoring/monitoring/analytics/index.md) | `analytics` | Loaded in `denied` mode via GA4 Consent Mode v2 | Script loads, but tags fire only after `gtag('consent', 'update', { analytics_storage: 'granted' })` |
| [PostHog](../../services/monitoring/monitoring/analytics/posthog.md) | `analytics` | `capture_pageview: false`, `persistence: 'memory'` until consent | After consent, persistence flips to `localStorage`; PostHog keeps anonymous mode by default per its service file |
| [Microsoft Clarity](../../services/monitoring/monitoring/analytics/microsoft-clarity.md) | `analytics` + `marketing` | Blocked until consent | Session-recording is the most sensitive surface; gated to both categories so denying either suppresses Clarity entirely |
| [Knock](../../services/business/push/knock.md) | `necessary` | Loaded always | Server-side transactional notifications; no client cookies |
| [FCM (web push)](../../services/business/push/fcm.md) | `functional` | Browser permission prompt only after user clicks "Enable notifications" | Consent for FCM is the OS-level Notifications permission, not a Klaro toggle; the toggle just shows the in-app prompt button |
| [Giscus](../../services/code/code-embed/index.md) | `social` | Loaded only after consent OR user-click on a "Load comments" placeholder | Lazy-loaded iframe; placeholder visible until clicked |
| [Algolia Insights](../../services/data/search/algolia.md) | `analytics` | Disabled until consent | Algolia search itself is `necessary`; the Insights events client is gated separately |
...
[jsDelivr](../../services/infra/cdn/jsdelivr.md)) ships **only** to
...
|---|---|---|
| [Cloudflare Web Analytics](../../services/monitoring/monitoring/analytics/cloudflare-web-analytics.md) | Cookie-less by design | Yes |
| [Sentry](../../services/monitoring/monitoring/sentry.md) | `sendDefaultPii: false` (default) | Yes |
| Cloudflare Pages basic auth (where used) | Token-based, no cookie | Yes |
| [PostHog](../../services/monitoring/monitoring/analytics/posthog.md) | `persistence: 'memory'` pre-consent | Yes (until consent flips it) |
| [reCAPTCHA Enterprise](../../services/business/auth/recaptcha-enterprise.md) | Cookie-less assessment mode | Yes (Firestore-write surface only) |
| [Cloudflare Turnstile](../../services/business/security/cloudflare-turnstile.md) | Cookie-less by design | Yes |
