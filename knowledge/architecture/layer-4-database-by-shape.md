...
description: Different data shapes go in different free tiers, deliberately spreading load so no single quota gets exhausted. Git for canonical, Firestore for user state, Turso for warm cache, browser for per-user search, R2 only when needed.
tags: [architecture, database, firestore, turso, jsonl, layer-4]
...
related:
  ...
  - architecture/events-table-schema
...
- The canonical store rule → [canonical-store-jsonl.md](canonical-store-jsonl.md)
- Why cloud DBs are caches not sources → [cloud-dbs-as-caches.md](cloud-dbs-as-caches.md)
- The events SQL schema → [events-table-schema.md](events-table-schema.md)
