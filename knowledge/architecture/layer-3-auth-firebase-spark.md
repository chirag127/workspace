...
description: One Firebase project (oriz-app) on the Spark plan, never Blaze. Custom auth domain auth.oriz.in shared by every site and every extension.
...
- Where user data lives → [layer-4-database-by-shape.md](layer-4-database-by-shape.md)
- The non-negotiable rule → [`../rules/no-card-on-file.md`](../rules/no-card-on-file.md)
