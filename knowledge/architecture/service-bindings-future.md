...
description: Cloudflare Service Bindings give zero-cost, zero-network-hop RPC between Workers. Reserved for a future split where the Hono umbrella Worker delegates privileged auth/billing logic to a separate "auth-core" Worker.
...
- The flow that needs the highest blast-radius isolation → [subscription-flow.md](subscription-flow.md)
