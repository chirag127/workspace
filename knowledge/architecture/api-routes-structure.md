...
description: The Hono Worker splits routes by concern under apps/api/src/routes/ — contact, recaptcha, razorpay, firestore, turso, auth. Each folder owns the integration with one external service.
tags: [architecture, api, hono, routes]
...
- `razorpay/` — webhook signature validation + write to
  `users/{uid}/subscription` in Firestore. See [subscription-flow.md](subscription-flow.md).
...
package isolation rule on the client side — see [package-isolation-rule.md](package-isolation-rule.md).

## Cross-refs

- The Worker hosting these routes → [api-umbrella-hono-worker.md](api-umbrella-hono-worker.md)
- How the Razorpay webhook reaches Firestore → [subscription-flow.md](subscription-flow.md)
- How auth verification works → [cross-site-auth-via-auth-oriz-in.md](cross-site-auth-via-auth-oriz-in.md)
