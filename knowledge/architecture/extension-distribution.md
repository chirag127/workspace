...
description: Every extension is its own GitHub repo, submoduled under extensions/. Each repo has its own CI workflow that publishes to Chrome Web Store, Firefox Add-ons, and Edge Add-ons on release. Landing pages live on extensions.oriz.in (with a copy at oriz.in/extensions).
...
- How extension auth shares state with sites → [cross-site-auth-via-auth-oriz-in.md](cross-site-auth-via-auth-oriz-in.md)
- The GitHub-Pages-as-fallback pattern → [layer-2-survival-fallback.md](layer-2-survival-fallback.md)
