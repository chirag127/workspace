...
description: The chirag127/oriz master repo's submodule pointers IS the production state of the family. Bumping a submodule pointer + pushing master = deploying that submodule to production via the matrix workflow.
...
- The submodule mechanics underneath → [submodule-pattern.md](submodule-pattern.md)
- The full repo layout → [repo-layout.md](repo-layout.md)
- Why the Worker is inline not submoduled → [api-umbrella-hono-worker.md](api-umbrella-hono-worker.md)
- The host the matrix deploys to → [layer-1-static-hosting.md](layer-1-static-hosting.md)
