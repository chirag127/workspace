...
description: Every site builds a static fallback to chirag127.github.io/<site> on every push to main. If Cloudflare Pages dies, /work + /me + /legal still serve from github.io. Per the 100-year strategy §16.
...
- Extension catalog landing pages can run on GitHub Pages full-time
  (not just as fallback) — see [extension-distribution.md](extension-distribution.md)
...
- Why git is canonical (same survival logic at the data layer) → [canonical-store-jsonl.md](canonical-store-jsonl.md)
- Extensions catalog hosting → [extension-distribution.md](extension-distribution.md)
